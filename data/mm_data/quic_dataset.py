...
class QuICDataset(OFADataset):
    ...
    def __getitem__(self, index):
        # dataset.columns also include "uniq_id", "query", "image(base64)", "caption"
        uniq_id, topic, image, caption = self.dataset[index]
        ...
        # without using queries for prompt
        # src_item = self.encode_text(self.prompt)
        
        # with using queries for prompt
        src_item = self.encode_text(self.prompt.strip("?") + f" about {topic}?")
